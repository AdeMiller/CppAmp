...
HRESULT CDXUTTextHelper::DrawTextLine( const WCHAR* strMsg )
{
    ...
    else if( m_pd3d11DeviceContext ) // This is called only for the display area caption
        DrawText11DXUTCaptions( m_pd3d11Device, m_pd3d11DeviceContext, strMsg, rc, m_clr,
                        (float)m_pManager->m_nBackBufferWidth, (float)m_pManager->m_nBackBufferHeight, false );

    if( FAILED( hr ) )
        return DXTRACE_ERR_MSGBOX( L"DrawText", hr );

    //m_pt.y += m_nLineHeight+5;
	m_pt.y += m_nLineHeight+10; // This is called only for the display area caption
